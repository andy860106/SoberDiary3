		...
		if (type == TYPE_FAMILY) {
			...
			for (int i = 0; i < 3; ++i) {
				if (connectName[i].length() > 0) {
					setSelectItem(connectName[i], new SelectedListener(msgBox,
							new CallCheckOnClickListener(msgBox,
									connectName[i], connectPhone[i]),
							R.string.next));
					++counter;
				}
			}
			...
			if (counter == 0) {
				...
				msgBox.setNextButton(R.string.ok,
						new CloseClickListener(msgBox));
			}
			...
			setSelectItem(n0,
					new SelectedListener(msgBox, new CallCheckOnClickListener(
							msgBox, n0, p0), R.string.next));
			setSelectItem(n1,
					new SelectedListener(msgBox, new CallCheckOnClickListener(
							msgBox, n1, p1), R.string.next));
			setSelectItem(n2,
					new SelectedListener(msgBox, new CallCheckOnClickListener(
							msgBox, n2, p2), R.string.next));
		}
	}
