...
public class DevelopNormalView extends StatisticPageView {
	...
	public void load() {
		...
		textFile = new File(mainStorageDir.getPath() + File.separator
				+ timestamp + File.separator + timestamp + ".txt");
	...
	protected boolean parseTextFile(File textFile) {
		try {
			...
			Double[] values_brac = valueArray_brac
					.toArray(new Double[valueArray_brac.size()]);
			Double[] values_voltage = valueArray_voltage
					.toArray(new Double[valueArray_voltage.size()]);
