...
import ubicomp.soberdiary3.data.structure.BreathDetail;
...
import ubicomp.soberdiary3.test.data.BreathDetailHandler;
...
public class Bluetooth {
	...
	protected final static UUID uuid = UUID
	 ...
	 */
	public Bluetooth(BluetoothDebugger debugger,
			BluetoothMessageUpdater updater, CameraRunHandler cameraRunHandler,
		...
		start = false;
		if (recordDetail) {
			bracPressureHandler = new BreathDetailHandler(
					bracFileHandler.getDirectory());
	...
	public void enableAdapter() {
		...
		if (!btAdapter.isEnabled()) {
			...
			btAdapter.enable();
			int state = btAdapter.getState(); // Possible return values are
												// STATE_OFF, STATE_TURNING_ON,
	...
	public boolean pair() {
		...
		/*
		 * if (sensor == null) { IntentFilter filter = new
		 * IntentFilter(BluetoothDevice.ACTION_FOUND); BroadcastReceiver
		 * receiver = new btReceiver(); context.registerReceiver(receiver,
		 * filter); btAdapter.startDiscovery(); }
		 */
		// return false;
	}
	...
	protected class btReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if (BluetoothDevice.ACTION_FOUND.equals(intent.getAction())) {
				...
				if (name == null)
					return;
				if (name.startsWith(DEVICE_NAME_FORMAL_OLD)
						|| name.startsWith(DEVICE_NAME_FORMAL_NEW)) {
		...
		try {
			...
			while (bytes >= 0) {
				...
				if (in.available() > 0) {
					...
					Log.d(TAG, "ZERO_DURATION=" + disconnectionMillis + " "
							+ try_time + " " + sleepTime);
				}
			}
			closeSuccess();
	...
	protected void handleException(Exception e) {
		...
		if (e.getMessage() != null && e.getMessage().equals(EXCEPTION_TIME_OUT)) {
			cameraRunHandler.sendEmptyMessage(3);
				...
				&& e.getMessage().equals(EXCEPTION_BLOW_TWICE)) {
			cameraRunHandler.sendEmptyMessage(4);
		} else if (e.getMessage() != null
				&& e.getMessage().equals(EXCEPTION_DISCONNECTION)) {
			cameraRunHandler.sendEmptyMessage(5);
		} else if (e.getMessage() != null
				&& e.getMessage().equals(EXCEPTION_PRESSURE_ERROR)) {
			cameraRunHandler.sendEmptyMessage(6);
		} else if (e.getMessage() != null
				&& e.getMessage().equals(EXCEPTION_HIGH_INITIAL_CONDITION)) {
	...
	protected boolean parseMsg(String msg) throws Exception {
		synchronized (lock) {
			...
			if (msg == "")
				...
				float diff_limit = PRESSURE_DIFF_MIN_RANGE
						* (5000.f - tempDuration) / 5000.f + PRESSURE_DIFF_MIN;
				if (pressureCurrent > pressureMin + diff_limit && !isPeak) {
					...
					if (totalDuration > MILLIS_5 && updateCircleTimes < 5) {
						showBrACCircle(5);
						updateCircleTimes = 5;
					} else if (totalDuration > MILLIS_4
							&& updateCircleTimes < 4) {
						showBrACCircle(4);
						updateCircleTimes = 4;
					} else if (totalDuration > MILLIS_3
							&& updateCircleTimes < 3) {
						showBrACCircle(3);
						updateCircleTimes = 3;
					} else if (totalDuration > MILLIS_2
							&& updateCircleTimes < 2) {
						showBrACCircle(2);
						updateCircleTimes = 2;
					} else if (totalDuration > MILLIS_1
							&& updateCircleTimes < 1) {
						showBrACCircle(1);
						updateCircleTimes = 1;
					}
					...
					if (imageCount == 0 && totalDuration > IMAGE_MILLIS_0) {
						...
						++imageCount;
							...
							&& totalDuration > IMAGE_MILLIS_1) {
						cameraRunHandler.sendEmptyMessage(0);
						++imageCount;
					} else if (imageCount == 2
							&& totalDuration > IMAGE_MILLIS_2) {
						cameraRunHandler.sendEmptyMessage(0);
						++imageCount;
					} else if (imageCount == 3
							&& totalDuration > MAX_DURATION_MILLIS) {
						showBrACCircle(6);
						return true;
					}
				...
				if (!start) {
					voltageInit = Integer.valueOf(msg.substring(1,
							msg.length() - 1));
				}
			} else if (msg.charAt(0) == 's') { // Serial number
				int serial = Integer
						.valueOf(msg.substring(1, msg.length() - 1));
			...
			if (bracPressureHandler != null && pressureList != null) {
				...
				if (serialList.size() > 0) {
					...
					for (int i = 1; i < serialList.size(); ++i) {
						...
						serialDiffMax = diff > serialDiffMax ? diff
								: serialDiffMax;
				...
				BreathDetail bd = new BreathDetail(bdTs, blowStartTimes,
						blowBreakTimes, pressureDiffMax, pressureMin,
						pressureAverage, voltageInit, disconnectionMillis,
						serialDiffMax, serialDiffAverage,
						PreferenceControl.getSensorID());
				DatabaseControl db = new DatabaseControl();
				db.insertBreathDetail(bd);

				data.putString("pressure", blowStartTimes + "\t"
						+ blowBreakTimes + "\t" + pressureAverage + "\t"
						+ pressureMin + "\t" + voltageInit + "\t" + "("
						+ disconnectionMillis + ")" + "\t(" + serialDiffMax
						+ "," + serialDiffAverage + ")");
				msg.setData(data);
				msg.what = 0;
