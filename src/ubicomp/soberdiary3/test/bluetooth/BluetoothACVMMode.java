...
public class BluetoothACVMMode extends Bluetooth {
	 ...
	 */
	public BluetoothACVMMode(BluetoothDebugger debugger,
		...
		synchronized (lock) {
			...
			// Do nothing
				...
				if (isPeak) {
					...
					String output = timeStamp + "\t" + temp_pressure + "\t"
							+ alcohol;
				...
				if (showPressureCount == 0)
					debugger.showDebug("p: " + pressureCurrent + " min: "
							+ pressureMin + " l:" + diff_limit);
				showPressureCount = (showPressureCount + 1)
						% SHOW_PRESSURE_COUNT_MOD;
