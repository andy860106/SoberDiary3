package ubicomp.soberdiary3.data.database;
...
import ubicomp.soberdiary3.data.structure.Detection3;
...
import ubicomp.soberdiary3.data.structure.Rank;
...
public class DatabaseControl3 {
	 ...
	 */
	public Detection3[] getAllPrimeDetection() {
		synchronized (sqlLock) {
			db = dbHelper.getReadableDatabase();
			String sql = "SELECT * FROM DETECTION3 WHERE isPrime = 1 ORDER BY ts ASC";
			...
			Detection3[] detections = new Detection3[count];
			for (int i = 0; i < count; ++i) {
				cursor.moveToPosition(i);
				float brac = cursor.getFloat(1);
				long ts = cursor.getLong(5);
				int trigger_type = cursor.getInt(8);
				int trigger_item = cursor.getInt(9);
				boolean isPrime = cursor.getInt(10) == 1;
				int weeklyScore = cursor.getInt(11);
				int score = cursor.getInt(12);
				detections[i] = new Detection3(brac, ts, trigger_type, trigger_item,
	 ...
	 * @see ubicomp.soberdiary.data.structure.Detection
	 */
	public Detection3 getLatestDetection() {
		synchronized (sqlLock) {
			db = dbHelper.getReadableDatabase();
			String sql = "SELECT * FROM DETECTION3 ORDER BY ts DESC LIMIT 1";
			Cursor cursor = db.rawQuery(sql, null);
			if (!cursor.moveToFirst()) {
				cursor.close();
				db.close();
				return new Detection3(0f, 0, -1, -1, false, 0, 0);
			}
			...
			int weeklyScore = cursor.getInt(11);
			int score = cursor.getInt(12);
			Detection3 detection = new Detection3(brac, ts, trigger_type, trigger_item,
					isPrime, weeklyScore, score);
	 ...
	 * @see ubicomp.soberdiary.data.structure.Detection
	 */
	public int insertDetection(Detection3 data, boolean update) {
		synchronized (sqlLock) {

			Detection3 prev_data = getLatestDetection();
	...
	public Detection[] getAllNotUploadedDetection() {
		synchronized (sqlLock) {
			...
			sql = "SELECT * FROM Detection3 WHERE upload = 0  ORDER BY ts ASC";
