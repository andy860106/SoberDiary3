package ubicomp.soberdiary3.system.uploader;
...
import ubicomp.soberdiary3.data.structure.AdditionalQuestionnaire;
import ubicomp.soberdiary3.data.structure.BreathDetail;
import ubicomp.soberdiary3.data.structure.Detection;
import ubicomp.soberdiary3.data.structure.EmotionDIY;
import ubicomp.soberdiary3.data.structure.EmotionManagement;
import ubicomp.soberdiary3.data.structure.ExchangeHistory;
import ubicomp.soberdiary3.data.structure.FacebookInfo;
import ubicomp.soberdiary3.data.structure.GCMRead;
import ubicomp.soberdiary3.data.structure.Questionnaire;
import ubicomp.soberdiary3.data.structure.StorytellingRead;
import ubicomp.soberdiary3.data.structure.StorytellingTest;
...
public class HttpPostGenerator {
	...
	public static HttpPost genPost() {
		...
		nvps.add(new BasicNameValuePair("userData[]", PreferenceControl
				.getSensorID()));
	...
	public static HttpPost genPost(Detection data) {
		...
		builder.addTextBody("data[]",
				String.valueOf(data.getTv().getTimestamp()));
		...
		geoFile = new File(mainStorageDir.getPath() + File.separator + _ts
				+ File.separator + "geo.txt");
		detectionFile = new File(mainStorageDir.getPath() + File.separator
				+ _ts + File.separator + "detection_detail.txt");
		geoAccuracyFile = new File(mainStorageDir.getPath() + File.separator
				+ _ts + File.separator + "geoAccuracy.txt");
	...
	public static HttpPost genPost(EmotionManagement data) {
		SERVER_URL_EMOTION_MANAGEMENT = ServerUrl
				.SERVER_URL_EMOTION_MANAGEMENT();
	...
	public static HttpPost genPost(UserVoiceRecord data) {
		...
		String uid = PreferenceControl.getUID();
		File mainStorageDir = new File(MainStorage.getMainStorageDirectory(),
				"audio_records");
		...
		builder.addTextBody("data[]", String.valueOf(data.getTv().getWeek()));
		builder.addTextBody("data[]",
				String.valueOf(data.getRecordTv().getYear()));
		builder.addTextBody("data[]",
				String.valueOf(data.getRecordTv().getMonth() + 1));
		builder.addTextBody("data[]",
				String.valueOf(data.getRecordTv().getDay()));
		builder.addTextBody("data[]", String.valueOf(data.getScore()));
	...
	public static HttpPost genPost(AdditionalQuestionnaire data) {
		SERVER_URL_ADDITIONAL_QUESTIONNAIRE = ServerUrl
				.SERVER_URL_ADDITIONAL_QUESTIONNAIRE();
	...
	public static HttpPost genPost(UserVoiceFeedback data) {
		...
		builder.addTextBody("data[]",
				String.valueOf(data.getTv().getTimestamp()));
		builder.addTextBody("data[]",
				String.valueOf(data.getDetectionTv().getTimestamp()));
		builder.addTextBody("data[]",
				String.valueOf(data.isTestSuccess() ? 1 : 0));
		builder.addTextBody("data[]", String.valueOf(data.hasData() ? 1 : 0));
		if (mainStorageDir != null) {
			File audio = new File(mainStorageDir, data.getDetectionTv()
					.getTimestamp() + ".3gp");
	...
	public static HttpPost genPost(BreathDetail data) {
		...
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data
				.getVoltageInit())));
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data
				.getDisconnectionMillis())));
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data
				.getSerialDiffMax())));
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data
				.getSerialDiffAverage())));
		nvps.add(new BasicNameValuePair("data[]", String.valueOf(data
				.getSensorId())));
		try {
			httpPost.setEntity(new UrlEncodedFormEntity(nvps, HTTP.UTF_8));
