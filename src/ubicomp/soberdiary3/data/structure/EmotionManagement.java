...
public class EmotionManagement {
	...
	public EmotionManagement(long ts, int rYear, int rMonth, int rDay,
			int emotion, int type, String reason, int score) {
	...
	public EmotionManagement(long ts, int rYear, int rMonth, int rDay,
			int emotion, int type, String reason, int score, boolean ver1) {
		this.tv = TimeValue.generate(ts);
		Calendar cal = Calendar.getInstance();
		cal.set(rYear, rMonth, rDay, 0, 0, 0);
		cal.set(Calendar.MILLISECOND, 0);
