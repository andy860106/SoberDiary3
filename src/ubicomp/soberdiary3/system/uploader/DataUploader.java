...
import ubicomp.soberdiary3.system.check.NetworkCheck;
import ubicomp.soberdiary3.system.cleaner.Cleaner;
...
public class DataUploader {
	...
	public static class DataUploadTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... arg0) {
			...
			AdditionalQuestionnaire[] addq = db
					.getNotUploadedAdditionalQuestionnaire();
			if (addq != null) {
			...
			if (not_uploaded_files != null) {
				for (int i = 0; i < not_uploaded_files.length; ++i) {
					File logFile = new File(logDir.getPath(),
							not_uploaded_files[i]);
					if (logFile.exists()) {
			...
			public boolean accept(File arg0, String arg1) {
				if (arg1.equals("latest_uploaded"))
					...
					if (today.compareTo(arg1) > 0)
						if (_latestUpload == null
								|| (_latestUpload != null && (arg1
										.compareTo(_latestUpload)) > 0))
							return true;
					return false;
				}
			}
		...
		private int connectToServer(AdditionalQuestionnaire data) {
			try {
				...
				HttpPost httpPost = HttpPostGenerator.genPost(data);
				if (upload(httpClient, httpPost))
					db.setAdditionalQuestionnaireUploaded(data.getTv()
							.getTimestamp());
				else
					return ERROR;
			} catch (Exception e) {
		...
		private boolean upload(HttpClient httpClient, HttpPost httpPost) {
			...
			try {
				...
				if (result) {
					String response = res.handleResponse(httpResponse)
							.toString();
