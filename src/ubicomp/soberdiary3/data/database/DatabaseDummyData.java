...
public class DatabaseDummyData extends AsyncTask<Void, Void, Void> {
	...
	private void insertDummyData() {
		...
		while (startDate.before(curDate)) {
			...
			if (rand_num < 50) {
				Detection morning = new Detection(0.f, ts + MORNING_TIMESTAMP,
						3 + rand.nextInt(2), rand.nextInt(2), true, 0, 0);
				Detection afternoon = new Detection(0.f, ts
						+ AFTERNOON_TIMESTAMP, 3 + rand.nextInt(2),
						rand.nextInt(2), true, 0, 0);
				Detection night = new Detection(0.f, ts + NIGHT_TIMESTAMP,
						3 + rand.nextInt(2), rand.nextInt(2), true, 0, 0);
				...
				Detection morning = new Detection(0.f, ts + MORNING_TIMESTAMP,
						2 + rand.nextInt(3), rand.nextInt(3), true, 0, 0);
				Detection afternoon = new Detection(0.f, ts
						+ AFTERNOON_TIMESTAMP, 2 + rand.nextInt(3),
				...
				Detection morning = new Detection(0.f, ts + MORNING_TIMESTAMP,
						2 + rand.nextInt(2), 2 + rand.nextInt(3), true, 0, 0);
				Detection afternoon = new Detection(0.f, ts
						+ AFTERNOON_TIMESTAMP, 2 + rand.nextInt(2),
						2 + rand.nextInt(3), true, 0, 0);
				Detection night = new Detection(0.f, ts + NIGHT_TIMESTAMP,
						2 + rand.nextInt(2), 2 + rand.nextInt(3), true, 0, 0);
				int rand_timeslot = rand.nextInt(3);
				if (rand_timeslot == 0)
					morning = new Detection(brac, ts + MORNING_TIMESTAMP,
							rand.nextInt(2), 5 + rand.nextInt(3), true, 0, 0);
				else if (rand_timeslot == 1)
					afternoon = new Detection(brac, ts + AFTERNOON_TIMESTAMP,
							rand.nextInt(2), 5 + rand.nextInt(3), true, 0, 0);
				else
					night = new Detection(brac, ts + NIGHT_TIMESTAMP,
							rand.nextInt(2), 5 + rand.nextInt(3), true, 0, 0);
				...
				float brac = 0.06f + rand.nextFloat() * 0.5f;
				Detection morning = new Detection(brac, ts + MORNING_TIMESTAMP,
						rand.nextInt(2), 5 + rand.nextInt(3), true, 0, 0);
				brac = 0.06f + rand.nextFloat() * 0.5f;
				Detection afternoon = new Detection(brac, ts
						+ AFTERNOON_TIMESTAMP, rand.nextInt(2),
						5 + rand.nextInt(3), true, 0, 0);
				brac = 0.06f + rand.nextFloat() * 0.5f;
				Detection night = new Detection(brac, ts + NIGHT_TIMESTAMP,
						rand.nextInt(2), 5 + rand.nextInt(3), true, 0, 0);
				int rand_timeslot = rand.nextInt(3);
				if (rand_timeslot != 0) {
					db.insertDetection(morning, false);
